...
function attach_sockets(server) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on("negotiation:message", async (data) => {
        ...
        if (!socket.userId) {
          socket.emit("negotiation:error", { error: "Not authenticated", message: "Socket not authenticated" });
          return;
        }
        ...
        if (!negotiation || negotiation.status !== "active") {
          socket.emit("negotiation:error", { error: "Negotiation not found", message: "Negotiation not found" });
          return;        }
        // emit error if user not in current negotiation
        if (socket.userId !== negotiation.userId && socket.userId !== negotiation.businessId) {
          socket.emit("negotiation:error", { error: "Not part of this negotiation", message: "Not part of this negotiation" });
          return;        }
        ...
        if (!activeNegotiation || activeNegotiation.id !== negotiationId) {
          socket.emit("negotiation:error", { error: "Negotiation Mismatch", message: "User not part of this negotiation" });
          return;        }
