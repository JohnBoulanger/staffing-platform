...
class QualificationService {
    static async createQualification(data, userId) {
    ...
    static async getQualification(qualificationId, requester) {
        ...
        if (requester.role === "business" && qualification.status !== "approved") {
        ...
        if (requester.role !== "business") {
            ...
            response.user.suspended = qualification.user.account.suspended;
    ...
    static async updateQualification(data, qualificationId, requester) {
        ...
        if (status) {
            ...
            if (requester.role === "admin") {
                if (!["submitted", "revised"].includes(qualification.status) || !["approved", "rejected"].includes(status)) {
                    throw { type: "forbidden" };
                }
                newStatus = status;
            }
            ...
            // regular rules
                ...
                if (qualification.user.accountId !== requester.id) {
                    throw { type: "forbidden" };
                }

                const validTransition = (qualification.status === "created" && status === "submitted") ||
